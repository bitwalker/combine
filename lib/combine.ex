defmodule Combine do
  ...
  defp filter_ignores(element) when is_list(element) do
    element |> Stream.filter(&ignore_filter/1) |> Enum.map(&filter_ignores/1)
  end
  ...
  defp transform_state(state, options) do
    ...
    results =
      state.results
      |> Enum.reverse
      |> Stream.flat_map(fn element when is_list(element) -> element; element -> [element] end)
      |> Stream.filter(&ignore_filter/1)
      |> Enum.map(&filter_ignores/1)
      |> List.flatten
      |> IO.inspect
    if options.keyword do
      ...
      case {results, can_zip?} do
        {[h|tail], _} when is_list(h) ->
          Enum.map([h|tail], &Enum.zip(labels, &1))
        {_, true} ->
          Enum.zip(labels, results)
        _ ->
            raise("Can not label all parsed results")
      end
    else
      results
    end
